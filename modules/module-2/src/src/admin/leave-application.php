...
if (isset($_POST['submit'])) {
    ...
    if ((!empty($leavetype)) && (!empty($fromdate))) {
        ...
        $queryleaveinsert = mysqli_prepare(
            $conn,
            "INSERT INTO `leave_applications` (`id`,`first_name`,`leave_type`,`from_date`,`to_date`,`reason`,`isadmin`)
             VALUES (?,?,?,?,?,?,?)"
        );
        mysqli_stmt_bind_param($queryleaveinsert, "isssssi", $userid, $name, $leavetype, $fromdate, $todate, $inputreason, $role);
    ...
    if ((!empty($remtype)) && (!empty($filedon)) && (!empty($amount))) {
        $queryreminsert = mysqli_prepare(
            $conn,
            "INSERT INTO `reimbursments` (`id`,`first_name`,`type`,`filed_on`,`amount`)
             VALUES (?,?,?,?,?)"
        );
        mysqli_stmt_bind_param($queryreminsert, "issss", $userid, $firstname, $remtype, $filedon, $amount);
